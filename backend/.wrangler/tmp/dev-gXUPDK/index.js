...
// .wrangler/tmp/bundle-RrGd4F/checked-fetch.js
...
app.post("/api/auth/login", async (c) => {
  ...
  if (user) {
    const token = await sign2({ ...user, exp: Math.floor(Date.now() / 1e3) + 60 * 60 * 24 }, c.env.JWT_SECRET || "secret");
